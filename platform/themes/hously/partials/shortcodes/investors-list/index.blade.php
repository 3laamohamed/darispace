        ...
        <div class="flex items-center gap-3">
            <div>
                <select name="per_page" id="per-page" class="p-3 border rounded-lg cursor-pointer border-slate-300 focus-visible:outline-primary dark:text-slate-600">
                    ...
                    @foreach(RealEstateHelper::getInvestorsPerPageList() as $perPage)
...
<section class="relative">
    <div class="container">
        <div id="items-map" @class(['hidden' => (! request()->input('layout') == 'map') || ! $showMap])>
            {!! Theme::partial("real-estate.investors.items-map", compact('investors')) !!}
        </div>
        <div id="items-list" data-type="investor" @class(['hidden' => request()->input('layout') == 'map']) data-layout="{{ theme_option('investors_list_layout') }}" style="max-height: none; max-width: none">
            {!! Theme::partial("real-estate.investors.items-$currentLayout", compact('investors')) !!}
        </div>
    </div>
</section>

<div class="hidden overflow-hidden duration-500 ease-in-out bg-white shadow group rounded-xl dark:bg-slate-900 hover:shadow-xl dark:hover:shadow-xl dark:shadow-gray-700 dark:hover:shadow-gray-700" id="investor-grid-skeleton">
    <div class="relative">
...
<div class="hidden w-full mx-auto overflow-hidden duration-500 ease-in-out bg-white shadow group rounded-xl dark:bg-slate-900 hover:shadow-xl dark:hover:shadow-xl dark:shadow-gray-700 dark:hover:shadow-gray-700 lg:max-w-2xl" id="investor-list-skeleton">
    <div class="md:flex">
