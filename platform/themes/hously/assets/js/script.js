...
class SearchFilter {
    ...
    keyword() {
        ...
        $('.search-filter')
            .on('keyup', 'input[name="keyword"]', function () {
                const $currentTarget = $(this).closest('form')
                const $searchForm = $currentTarget.find('input[name="keyword"]').parent()

                $searchForm.find('.mdi-loading').removeClass('hidden')

                timeout = setTimeout(() => {
                    const keyword = $currentTarget.find('input[name="keyword"]').val()
            ...
            .on('keydown', 'input[name="keyword"]', function () {
